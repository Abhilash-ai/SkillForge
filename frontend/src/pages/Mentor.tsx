...
const Mentor = () => {
  ...
  return (
    <div className="h-screen bg-slate-50 flex overflow-hidden">
      ...
      <div className={`fixed inset-y-0 left-0 w-72 bg-white border-r border-slate-200 z-30 transform transition-transform duration-300 md:relative md:translate-x-0 flex flex-col ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-4 border-b border-slate-200 flex justify-between items-center bg-slate-50 shrink-0">
          <h2 className="font-bold text-slate-800">Chat History</h2>
          <button onClick={() => setSidebarOpen(false)} className="md:hidden text-slate-500 p-1">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-4 shrink-0">
          <button 
            ...
            className="w-full flex items-center justify-center gap-2 py-3 px-4 bg-white dark:bg-slate-900 hover:bg-slate-100 dark:bg-slate-800 text-white rounded-xl font-medium transition-colors"
          >
        ...
        <div className="flex-1 overflow-y-auto px-3 pb-4 space-y-1 scrollbar-hide">
          {sessions.map(s => (
            ...
            >
              <MessageSquare className={`w-4 h-4 shrink-0 ${activeSessionId === s.id ? 'text-cyan-500' : 'text-slate-600 dark:text-slate-400'}`} />
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="w-full bg-white border-b border-slate-200 px-4 md:px-8 py-4 flex justify-between items-center shrink-0 shadow-sm z-10">
          <div className="flex items-center gap-3">
            <button onClick={() => setSidebarOpen(true)} className="md:hidden p-2 -ml-2 text-slate-500 hover:text-slate-800 rounded-lg">
              <Menu className="w-6 h-6" />
            </button>
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-cyan-500 to-fuchsia-600 flex items-center justify-center shadow-lg shadow-cyan-500/30">
              <Bot className="w-6 h-6 text-white" />
            </div>
            <div>
              <h1 className="text-lg md:text-xl font-bold text-slate-900 flex items-center gap-2">
                SkillForge Mentor <Sparkles className="w-4 h-4 text-cyan-500 hidden sm:block" />
              </h1>
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-8 space-y-6 scroll-smooth bg-slate-50/50">
          <div className="max-w-3xl mx-auto space-y-6">
            {messages.length === 0 && !loading && (
              <div className="h-full flex flex-col items-center justify-center text-slate-600 dark:text-slate-400 mt-20">
                <Bot className="w-16 h-16 mb-4 text-slate-700 dark:text-slate-300" />
            ...
            {messages.map((msg, idx) => (
              ...
              >
                ...
                <div className={`p-4 rounded-2xl max-w-[85%] sm:max-w-[80%] ${msg.role === 'user' ? 'bg-fuchsia-600 text-white rounded-tr-sm' : 'bg-white border border-slate-200 shadow-sm text-slate-700 rounded-tl-sm'}`}>
          ...
          <div className="max-w-3xl mx-auto">
            <form onSubmit={handleSend} className="relative flex items-center">
              <input
                ...
                className="w-full pl-6 pr-16 py-4 rounded-full border border-slate-300 shadow-sm focus:outline-none focus:ring-2 focus:ring-cyan-500 bg-white transition-all text-slate-800 font-medium"
              />
              <button
                type="submit"
                disabled={!input.trim() || loading || !activeSessionId}
                className="absolute right-2 w-10 h-10 rounded-full bg-cyan-600 hover:bg-cyan-700 disabled:bg-slate-300 text-white flex items-center justify-center transition-colors shadow-md"
              >
