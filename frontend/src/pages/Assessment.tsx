...
const Assessment = () => {
        ...
        try {
          ...
          if (userTopics.length > 0) {
            const customCats = userTopics.map((t: string, i: number) => ({
              id: t, 
              icon: Target, 
              color: ['text-amber-500', 'text-rose-500', 'text-teal-500', 'text-fuchsia-500'][i % 4], 
              bg: ['bg-amber-500/10', 'bg-rose-500/10', 'bg-teal-500/10', 'bg-fuchsia-500/10'][i % 4]
            }));
  ...
  if (!selectedCategory) {
    return (
      <div className="min-h-screen p-8 text-zinc-900 dark:text-zinc-100 max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-4xl mx-auto">
          {categories.map((cat, idx) => (
            <motion.div
              ...
              className="forge-card p-6 cursor-pointer hover:border-amber-500/50 hover:bg-zinc-100 dark:bg-zinc-800/80 transition-all flex flex-col items-center text-center group"
            >
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#18181B] flex flex-col items-center justify-center text-zinc-900 dark:text-zinc-100">
        ...
        <p className="text-zinc-600 dark:text-zinc-400">Heating up the metal for your test in {selectedCategory}.</p>
      </div>
    );
  }
  ...
  if (questions.length === 0) {
    return (
      <div className="min-h-screen p-8 text-zinc-900 dark:text-zinc-100 flex flex-col items-center justify-center">
        <div className="forge-card p-8 text-center">
           <h2 className="text-2xl font-bold font-['Outfit'] text-red-500 mb-2">Forging Failed</h2>
           <p className="text-zinc-600 dark:text-zinc-400 mb-6">Could not generate questions. The fire is too weak today.</p>
           <button onClick={() => setSelectedCategory(null)} className="forge-button px-6 py-2 rounded-lg bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-700">Choose Another Category</button>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen p-8 text-zinc-900 dark:text-zinc-100 flex flex-col items-center justify-center">
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <p className="text-zinc-600 dark:text-zinc-400 mb-8 font-medium">Your skills in {selectedCategory} have been tested.</p>
              ...
              <>
                ...
                <div className="space-y-4 mb-8 text-left">
                  <div className="flex justify-between p-4 rounded-xl bg-[#27272A] border border-[#3F3F46]">
                    <span className="text-zinc-600 dark:text-zinc-400 font-medium">Final Score</span>
                    <span className="font-bold text-amber-500">{finalScore} / {maxPossibleScore} pts</span>
                  </div>
                  <div className="flex justify-between p-4 rounded-xl bg-[#27272A] border border-[#3F3F46]">
                    <span className="text-zinc-600 dark:text-zinc-400 font-medium">Correct / Wrong</span>
                    <span className="font-bold text-zinc-800 dark:text-zinc-200"><span className="text-emerald-500">{correctCount}</span> / <span className="text-red-500">{wrongCount}</span></span>
                  </div>
                </div>
                ...
                <div className="flex gap-4">
                  <button 
                    ...
                    className="flex-1 py-4 rounded-xl bg-zinc-100 dark:bg-zinc-800 border border-zinc-300 dark:border-zinc-700 text-white font-bold hover:bg-zinc-700 transition-colors"
                  >
                    New Assessment
                  </button>
                  <button 
    ...
    <div className="min-h-screen p-8 text-zinc-900 dark:text-zinc-100 flex items-center justify-center">
      <div className="max-w-3xl w-full">
        <div className="mb-8 flex items-center justify-between">
          <button onClick={() => setSelectedCategory(null)} className="text-zinc-600 dark:text-zinc-400 hover:text-white transition-colors">
            ← Change Category
          </button>
          <div className="flex items-center gap-4">
            <div className="text-zinc-600 dark:text-zinc-400 font-bold bg-white dark:bg-zinc-900 px-4 py-2 rounded-lg border border-zinc-200 dark:border-zinc-800">
              Score: <span className={score >= 0 ? 'text-amber-500' : 'text-red-500'}>{score}</span> pts
            </div>
            <div className="text-zinc-600 dark:text-zinc-400 font-bold font-['Outfit'] tracking-widest uppercase text-sm">
        ...
        >
          ...
          <h3 className="text-3xl font-black font-['Outfit'] text-zinc-900 dark:text-zinc-100 mb-10 mt-4 leading-tight">
            {q.question}
          </h3>
              ...
              <button
                ...
                className={`w-full p-5 rounded-xl text-left border-2 transition-all duration-200 flex justify-between items-center group ${
                  selected === i 
                    ...
                    : 'border-zinc-300 dark:border-zinc-700 bg-zinc-100 dark:bg-zinc-800/50 hover:border-zinc-500 hover:bg-zinc-100 dark:bg-zinc-800'
                }`}
              >
                <span className={`font-medium text-lg ${selected === i ? 'text-amber-500' : 'text-zinc-700 dark:text-zinc-300 group-hover:text-zinc-800 dark:text-zinc-200'}`}>{opt}</span>
                {selected === i && (
