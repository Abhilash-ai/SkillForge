...
const Sidebar = ({ isOpen, setIsOpen }: SidebarProps) => {
  ...
  return (
    <>
      ...
      {/* Sidebar Drawer */}
      <div className={`w-64 h-screen bg-white dark:bg-zinc-900 border-r border-zinc-200 dark:border-zinc-800 flex flex-col fixed left-0 top-0 shadow-2xl z-50 transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}>
        <div className="p-6 flex items-center justify-between border-b border-zinc-200 dark:border-zinc-800/60 shadow-[0_1px_2px_rgba(0,0,0,0.1)] shrink-0">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-orange-500 to-rose-600 flex items-center justify-center shadow-lg shadow-rose-500/30">
          ...
          <button 
            onClick={() => setIsOpen(false)} 
            className="md:hidden text-zinc-600 dark:text-zinc-400 hover:text-white p-1 rounded-lg hover:bg-zinc-100 dark:bg-zinc-800 transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 px-4 py-3 rounded-[14px] font-medium transition-all duration-200 ${
                  isActive
                    ...
                    : 'text-zinc-600 dark:text-zinc-400 hover:bg-zinc-100 dark:bg-zinc-800/80 hover:text-zinc-800 dark:text-zinc-200 border border-transparent'
                }`
              }
            >
